using ResourceManager.WebApp.Models;
using System;
...
using System.Web;
using ResourceManager.WebApp.App_Start;
using Microsoft.AspNet.Identity;

namespace ResourceManagement.WebApp
{
    public class ApplicationDbContext : IdentityDbContext<ApplicationUser>
    {
        ...
        protected override void OnModelCreating(DbModelBuilder modelBuilder)
        {
            ...
            {
                throw new ArgumentNullException("Model builder is found NULL");
            }
            base.OnModelCreating(modelBuilder);

            // Defining Keys and Realtionships

            modelBuilder.Entity<ApplicationUser>().ToTable("AspNetUsers");
            modelBuilder.Entity<ApplicationRole>().HasKey(key => key.Id).ToTable("AspNetRoles");
            modelBuilder.Entity<ApplicationUser>().HasMany<ApplicationUserRole>((ApplicationUser item) => item.UserRoles);

            modelBuilder.Entity<ApplicationUserRole>().HasKey(item => new
            {
                UserId = item.UserId,
                RoleId = item.RoleId
